...
calls — free beyond gas and permissionless. The publisher keeps the package
`UpgradeCap` for native package upgrades. Verify at
...
`isChainProvisioned` flips on from `moduleAddress`. Set the entry's
`integrationStatus` to match reality (`"testnet-deployed"`, then
`"mainnet-deployed"`) — never above what is actually deployed and verified.
