...
- `cargo test -p fileonchain-registry` passing in `contracts/near/`.
...
The contract state is `PanicOnDefault`, so deploy with the no-arg `new()`
init call:
...
Upgrades are NEAR-native: redeploy the wasm to the same account with its
full-access key (`without-init-call` on redeploys — state is preserved).
...
on from `moduleAddress`. Set the entry's `integrationStatus` to match
reality (`"testnet-deployed"`, then `"mainnet-deployed"`) — never above
